document.addEventListener("DOMContentLoaded", () => {
  ...
  const validatePassword = (password) => {
    ...
    if (!/[@#$%&*!?/\\|_\+\.\-=]/.test(password))
      return "A senha deve conter um caractere especial permitido.";
    if (/[¨{}[\]´`~^:;<>,“‘,]/.test(password))
  ...
  form.addEventListener("submit", (event) => {
    ...
    if (nameParts.length < 2 || nameParts[0].length < 2) {
      showMessage(
        "O nome deve ter pelo menos duas palavras, com a primeira tendo no mínimo 2 caracteres.",
        "error"
      );
    ...
    if (!isValidPhone(phoneInput.value)) {
      showMessage("O telefone deve estar em um formato válido.", "error");
  ...
  clearButton.addEventListener("click", () => {
    ...
    document.querySelector(
      'input[name="maritalStatus"][value="solteiro"]'
    ).checked = true;
