document.addEventListener('DOMContentLoaded', () => {
    ...
    function renderTable() {
        requestsTableBody.innerHTML = ''; // Limpa a tabela
        initialRequests.sort((a, b) => new Date(a.date) - new Date(b.date)); // Ordena
    ...
    function handleAddRequest(event) {
        ...
        const newIdNumber = initialRequests.length + 3; // Simples geração de ID
        const newRequest = {
    ...
    loadUserData();
    populateServices();
    updateServiceDetails(); // Chama para carregar os detalhes do primeiro serviço
    renderTable();
